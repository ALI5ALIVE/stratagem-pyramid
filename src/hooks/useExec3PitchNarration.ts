...
export const useExec3PitchNarration = () => {
  ...
  const fetchAudio = async (slideId: string): Promise<string> => {
    ...
    if (!narration) throw new Error(`No Exec3 narration for slide ${slideId}`);
  ...
  const play = useCallback(
    async (slideId: string) => {
      ...
      try {
        ...
        console.error("Failed to play Exec3 narration:", error);
