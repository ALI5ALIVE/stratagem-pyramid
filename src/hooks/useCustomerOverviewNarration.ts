...
export const useCustomerOverviewNarration = () => {
  ...
  const fetchAudio = async (slideId: string): Promise<string> => {
    ...
    const narration = getCustomerOverviewNarration(slideId);
    if (!narration) throw new Error(`No CO narration for slide ${slideId}`);
  ...
  const play = useCallback(
    async (slideId: string) => {
      ...
      try {
        ...
        console.error("Failed to play CO narration:", error);
