...
import { getExecPitchNarration } from "@/data/executivePitchNarration";
...
export const useExecPitchNarration = () => {
  ...
  const fetchAudio = async (slideId: string): Promise<string> => {
    ...
    const narration = getExecPitchNarration(slideId);
    if (!narration) {
      throw new Error(`No exec narration found for slide ${slideId}`);
    }
  ...
  const play = useCallback(async (slideId: string) => {
    ...
    try {
      ...
      console.error("Failed to play exec narration:", error);
      setState(prev => ({
        ...prev,
        isPlaying: false,
        isLoading: false,
