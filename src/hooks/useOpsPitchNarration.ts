...
export const useOpsPitchNarration = () => {
  ...
  const fetchAudio = async (slideId: string): Promise<string> => {
    ...
    const narration = getOpsPitchNarration(slideId);
    if (!narration) {
      throw new Error(`No ops narration found for slide ${slideId}`);
    }
  ...
  const play = useCallback(async (slideId: string) => {
    ...
    try {
      ...
      console.error("Failed to play ops narration:", error);
      setState(prev => ({
        ...prev,
        isPlaying: false,
        isLoading: false,
