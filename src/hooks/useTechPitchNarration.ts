...
export const useTechPitchNarration = () => {
  ...
  const fetchAudio = async (slideId: string): Promise<string> => {
    ...
    const narration = getTechPitchNarration(slideId);
    if (!narration) {
      throw new Error(`No tech narration found for slide ${slideId}`);
    }
  ...
  const play = useCallback(async (slideId: string) => {
    ...
    try {
      ...
      console.error("Failed to play tech narration:", error);
      setState(prev => ({
        ...prev,
        isPlaying: false,
        isLoading: false,
