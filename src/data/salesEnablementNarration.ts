...
export const salesEnablementNarrations: SESlideNarration[] = [
  ...
  },
  {
    ...
    voiceId: VOICE,
    script:
      "Week 2 is the longest week because the capabilities are where the walkthrough lives. By the end of it you should be able to give a sixty-second walk-through of each capability and ask one good discovery question per capability. We go in this order: the Platform map, then the headline platform-wide Insights and Intelligence capability, then the three platform-wide use cases only that layer can answer, then we open the box with the Intelligence Layer and the per-solution use cases inside each Core App, then Insights, then Recommendations use cases, then Automation and its use cases, then Intelligence Layer versus generic AI, then Regulation Management as your end-to-end proof case, then the Unified Mobile App, and we close with the capability talk track. DTOP itself you already learned in Week 1 — this week you map each capability onto the loop. Take it slowly. This is the week reps under-prepare for and lose deals over.",
  },
  {
  ...
  },
  {
    ...
    voiceId: VOICE,
    script:
      "Why this slide matters: this is the single most powerful capability story in the deck — a platform-wide intelligence capability that any user can trigger just by asking a question. Core message: this isn't a chatbot bolted onto a dashboard. Any user asks an operational question in plain English — for example, are dangerous goods incidents linked to training gaps — and the platform returns a cross-domain answer with recommended actions in seconds. In the example, three stations show DG handling spikes, all three correlate with overdue DG recurrent training, and the recommended actions are generated automatically. The pain it addresses: today this question takes a director two weeks, a BI ticket, three exports and a spreadsheet — and the answer still doesn't connect safety to training. The value lever — and you must land this verbatim — approximately ninety percent domain accuracy on aviation operational questions versus around thirty-five percent for generic AI tools pointed at the same data, because we sit on connected operational data, a four-thousand-plus aviation knowledge graph at five levels, and domain-trained reasoning with cited evidence — not a generic LLM guessing. How to deliver it: walk the six stages left to right — plain-English question, connected operational data, domain knowledge graph, domain-trained reasoning, guardrails and audit trail, then answer plus recommended actions. Emphasise tenant isolation and source citations every time — that handles the security objection before it's asked. Discovery question: what's the cross-domain question your team can never get a straight answer to today? Their answer is the demo you book next. Transition: next we make this concrete — the three platform-wide use cases only this layer can answer.",
  },
  {
    ...
    voiceId: VOICE,
    script:
      "Why this slide matters: this is where Intelligence and Insights stops being a feature pitch and becomes a board-level capability. Three questions only the platform layer can answer — show me a correlation between recent safety trends and training deficiencies, how is the Dangerous Goods manual update landing in incident rates, are we ready for Part 145. Core message: no single Core App can answer these — they require the unified operational data substrate plus the intelligence layer on top. The pain you're addressing: today these questions take weeks of BI tickets and still come back ambiguous. The value lever: cross-domain answers in seconds, with cited evidence and recommended next actions tied to DTOP. How to deliver it: pick the question that fits the room — safety-and-training for a Director of Safety, the Dangerous Goods loop for a Head of Training, Part 145 for a Quality leader — and walk one card end to end. Point at the DTOP chips on each card so the customer sees the loop close. Discovery question: which of these three cross-domain questions does your leadership ask most often without a straight answer? Transition: next we open the box — the Intelligence Layer that actually runs these answers.",
  },
  {
  ...
  },
  {
    ...
    voiceId: VOICE,
    script:
      "Why this slide matters: prospects buy capabilities they can picture themselves using on a Monday morning. This is the picture for Intelligence and Insights inside each Core App. The core message: at the solution level, Intelligence and Insights does the chores nobody has time for — pulling a lessor-ready safety report for one tail number, listing the crew who haven't synced in thirty days, scheduling renewals at one base. The pain you're addressing: today these are spreadsheet jobs that consume a coordinator's week and still ship late. The value lever: the same capability that answers the board-level questions you just walked also clears the day-to-day chores — same engine, two different audiences. Delivery tip — pick the example that matches your prospect's role and stop talking. Discovery question: what's the report your team rebuilds every month that should already exist? Transition: next, Insights — how the platform earns trust by surfacing patterns before it ever automates.",
  },
  {
