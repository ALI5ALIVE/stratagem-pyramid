...
export const salesEnablementNarrations: SESlideNarration[] = [
  ...
  },
  {
    ...
    voiceId: VOICE,
    script:
      "Before we open the sell-and-win playbook, we recap the loop — because everything in Week 3 only works if you can frame the value of the platform and the capabilities inside it in sixty seconds. Why it matters: customers don't buy capabilities, they buy outcomes — and the rep's job in every first conversation is to make sure the customer understands the platform is one loop, not a list of products. The core message — one sentence, learn it verbatim: one use case, every capability, one DTOP loop. Walk the Madrid unstable approach trend end-to-end. Day one — Detect. Operational Data and Generative AI surface the rising trend; no analyst exported a CSV. Day two — Trigger. The Intelligence Layer answers where attention should be today with three recommended risk controls and cited evidence. Day three — Orchestrate. Automation drafts the OMA revision, opens the SafetyManager365 risk-control review, and notifies training owners; Unified Mobile pushes the revised procedure to crew devices and confirms sync. Day five — Prove. Insights shows the trend flat and the audit pack is one click. That is every Week 2 capability, inside one loop, end-to-end, in five days. The pain you're framing for the customer: today this same scenario takes their team three weeks across five disconnected tools and they still can't prove the loop closed. The value lever: the platform is the loop — Core Apps for the system of record, the Intelligence Layer for the decision, Automation and Unified Mobile for the action, Insights for the proof. How to deliver it: draw the loop first, name the capability inside each cell second, only then tell the story. End on Prove, then pause. Drill this until you can recite it in sixty seconds without looking at the slide — it is the frame you walk into every Week 3 conversation with. Next: who to target — the accounts where this loop lands fastest.",
  },
  {
