...
export const salesEnablementNarrations: SESlideNarration[] = [
  ...
  },
  {
    ...
    voiceId: VOICE,
    script:
      "Why this slide matters: by now you can size the footprint and name the broken loop — but the deals that compound are the ones where the customer sees the whole vision in one picture, not a feature list across six slides. This is the whiteboard that does that. Core message — learn it verbatim: one foundation, three Core Apps, one Intelligence and Orchestration layer, one Unified Mobile, all closed by DTOP. Draw it bottom-up and anchor it to one use case — the Madrid unstable approach trend works in almost any room. The pain you're addressing: prospects have been sold capability stacks before, and every vendor's diagram looks the same — boxes on top of boxes. They cannot picture the value because nobody has tied the stack to a single, believable outcome. The value lever: when you draw the stack bottom-up and walk one use case up through it, the customer experiences the platform as one decision, not five — Core Apps unify the foundation, Insights makes the trend surface itself, the Intelligence Layer reasons over it at roughly ninety percent domain accuracy versus around thirty-five percent for generic AI, Recommendations and Prescriptive Actions tells the team what to do next with cited evidence, and Automation plus Unified Mobile lands the change on crew devices in 48 hours — Prove takes care of itself. How to deliver it: ninety seconds, one marker, bottom-up — first the three Core Apps as the foundation, then the Intelligence and Orchestration band with Insights, Intelligence, and Recommendations and Prescriptive Actions, then the Automation band, then Unified Mobile, then the DTOP loop wrapping it all. Use one line per layer — the say-it script on the right is the script — then tap the use-case strip at the bottom and walk the same MAD trend from foundation to Prove without pausing. End with the close: start with one app, the stack lifts the rest as you grow. Then stop talking. Approved terminology only — Generative AI, Recommended Actions, Operational Data — no FOQA, FDM or ASAP, and the Intelligence Layer accuracy headline is the canonical roughly ninety versus thirty-five framing. Transition: now that the whole vision is on the board, the rest of Week 3 is about the motion that gets you back into that room with the right people — Discovery, Walkthrough, and Close.",
  },
  {
