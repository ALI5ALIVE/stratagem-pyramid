...
export const salesEnablementNarrations: SESlideNarration[] = [
  ...
  },
  {
    ...
    voiceId: VOICE,
    script:
      "One marker. Ninety seconds. Bottom-up. That's the whole drill. Core message — learn it verbatim: every layer is a deal on its own; stacked, they're the only loop that closes itself. The pain you're addressing: buyers have seen capability stacks before — boxes on boxes, no entry point and no compounding outcome. This whiteboard fixes that by giving them four moves in order, each one buyable on its own. Walk the four beats on the right of the slide, in order, and read the say-it line as it's written — don't paraphrase. One: Core Apps — the foundation, Detect starts here. SafetyManager365, ContentManager365, TrainingManager365 — one signal, visible to all three. Two: Insights and Intelligence — Detect into Trigger. Trends surface themselves; a domain-trained Intelligence Layer answers at roughly ninety percent accuracy versus around thirty-five for generic AI, every answer cited. Three: Recommendations and Prescriptive Actions — closes Trigger. Recommended Actions with evidence attached, not another dashboard. Four: Automation plus Unified Mobile — Orchestrate and Prove. Drafts the revision, opens the review, assigns the training, lands on crew devices, audit pack writes itself. The walk: now trace one finger up the board — Madrid unstable approach trend, Detected on day one, Triggered on day two, Orchestrated to crew devices by day three, Proven flat in five — one breath, no pauses. Close on the line at the bottom of the slide, verbatim: 'that's the whole vision — start with one app, the stack lifts the rest as you grow. One foundation, one intelligence, one loop.' Then stop talking. Guardrails: approved terminology only — Generative AI, Recommended Actions, Operational Data, Intelligence Layer — and the accuracy headline stays at the canonical ninety versus thirty-five. Transition: platform's on the board — the next slide answers the two questions every buyer asks the moment they've seen it: what do you mean by signals, and what is this DTOP loop. Recap picks it up from there.",
  },
  {
