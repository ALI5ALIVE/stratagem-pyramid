...
export const salesEnablementNarrations: SESlideNarration[] = [
  ...
  },
  {
    ...
    voiceId: VOICE,
    script:
      "Week 2 is the longest week because the capabilities are where the walkthrough lives. By the end of it you can give a sixty-second walk-through of each capability and ask one good discovery question per capability. The order matters and matches the deck exactly. One — the Platform map. Two — the platform-wide Insights and Intelligence capability, our headline story. Three — the three platform-wide use cases only that layer can answer. Four — the Intelligence Layer deep-dive. Five — the per-solution use cases inside each Core App. Six — Insights. Seven — the Recommendations and Prescriptive Actions use cases. Eight — Automation. Nine — the Automation use cases. Ten — Intelligence Layer versus generic AI, your defensive slide. Eleven — Regulation Management as the cleanest end-to-end proof case. Twelve — the Unified Mobile App. Thirteen — the capability talk track. DTOP itself you already learned in Week 1; this week you map each capability onto the loop. Take it slowly — this is the week reps under-prepare for and lose deals over.",
  },
  {
  ...
  },
  {
    ...
    voiceId: VOICE,
    script:
      "Why this slide matters: you have just shown the platform-wide capability and the three questions it can answer; this is the slide where you explain what is actually inside the box. Core message: the Intelligence Layer is the reasoning engine that sits on top of the connected platform. It is grounded in the customer's own operational schema and our aviation taxonomy, it turns operational data into insights, recommendations and triggered actions, and every answer it returns carries its evidence. The pain it addresses: their teams are drowning in dashboards and starving for answers, and every generic-AI experiment they have run has produced plausible-sounding output that did not survive scrutiny — because the model was not grounded in their data. The value lever: this is the layer that takes the three board-level questions you just demonstrated and runs them end to end — connected data plus knowledge graph plus domain-trained reasoning plus guardrails. Delivery tip — never call it a chatbot. Call it an analyst that lives inside your operation. The accuracy headline lives on slide eleven; do not reuse it here or you blunt it when you actually need it. Discovery question: who on your team spends the most time pulling reports that should already exist? Transition: that is what the layer is. Next slide, what it looks like on a Monday morning — the per-solution use cases inside each Core App that any coordinator can picture themselves running.",
  },
  {
