...
export const salesEnablementNarrations: SESlideNarration[] = [
  ...
  },
  {
    ...
    voiceId: VOICE,
    script:
      "One marker. Ninety seconds. Start at the top of the board, end at the bottom, and use Madrid as the thread the whole way down. Point at the use-case strip first — Madrid unstable approach trend, closed in five days — and walk the four DTOP micro-cards across the top so the buyer sees the worked example before they read the stack. Detect day one, Trigger day two, Orchestrate day three, Prove day five. That's the answer; the rest of the board is how we get there. Core line, verbatim: every layer is a deal on its own; stacked, they're the only loop that closes itself. Now bottom-up, five beats. One — Core Apps. The foundation. SafetyManager365, ContentManager365, TrainingManager365 — one signal, three systems of record, visible to all three. Two — Insights, in blue, the Detect box. The Madrid trend surfaces itself in fourteen days; no analyst pulled the report. Three — the Intelligence Layer, in amber, the Trigger box. Roughly ninety percent accurate on aviation work versus around thirty-five for generic AI, every answer cited to regulation, procedure and training — and it comes back as Recommended Actions, not another dashboard. For Madrid: revise the OMA, retrain four crews, evidence attached. Four — Automation and Unified Mobile, in violet, the Orchestrate box. Drafts the OMA revision, opens the reviews, assigns the training, lands on crew devices in forty-eight hours. Five — Insights again, in green, the Prove box. Madrid trend flat in five days, audit pack one click, loop closed. Close, verbatim: one foundation, one intelligence, one loop — every layer a deal on its own. Hand straight off to Recap.",
  },
  {
