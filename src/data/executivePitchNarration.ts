...
export const exec3PitchNarrations: ExecNarrationSlide[] = [
  ...
  },
  {
    ...
    voiceId: DEFAULT_VOICE_ID,
    script:
      "This capability is about one thing — when somebody in your business has a question, they get an answer they can act on, today. Right now, that loop is broken. Your COO asks a question on Monday, the BI team builds a report, and three weeks later the answer arrives after the operation has already moved on. We close that gap. Anyone — a duty manager, a safety lead, a chief pilot — types a question in plain English, in the platform, and in under a minute they get a cited answer with the source data, the reasoning, and the single next best step tied to that question. The engine behind it is CoAnalyst — and this is where the ninety percent versus thirty-five percent number lives in the real world. Unlike generic AI tools, which hallucinate the moment you ask them to classify aviation events at any depth, and unlike competitor bolt-on AI features that are limited to basic summarisation, CoAnalyst delivers aviation-specific precision built on a hybrid AI architecture, millions of historic reports going back to twenty twenty-three, and continuous R&D. In plain terms — ask 'show me the bird-strike trend at our top three hubs over the last ninety days,' and CoAnalyst returns the right answer with sources and the next best step, where a generic model would mis-classify the events and miss the cluster entirely. This is reactive intelligence — you ask, the platform answers — and it is live today on SafetyManager365, with content and training following. The next slide is what comes when your platform stops waiting to be asked.",
  },
  {
  ...
  },
  {
    ...
    voiceId: DEFAULT_VOICE_ID,
    script:
      "Today, when a new directive lands, your team starts a four-to-six-week scramble — read the directive, find every affected manual, draft the changes, route to SMEs, push to the fleet, chase acknowledgements, then prepare the audit thread. Most of that time is plumbing, not judgement. With this platform, the directive arrives, the platform maps it against your procedure library and identifies every affected document. Updates are queued for the right SME, automation tracks the cascade, every affected crew member receives the updated version and acknowledges it on the same mobile shell they already use, and the audit trail builds itself. Weeks of work compressed to days, and your compliance team gets to focus on judgement instead of chasing inboxes. Now — what's locked and committed for next year.",
  },
  {
