...
import { howItWorksSteps, AutomationUseCase } from "@/data/automationPlaybook";
import { MessageSquareQuote, GraduationCap, Workflow, Plug } from "lucide-react";

const iconMap: Record<string, React.ElementType> = { GraduationCap, Workflow, Plug };
const stepLabels = ["Trigger", "Orchestrate", "Execute", "Outcome"] as const;
const stepKeys = ["ask", "correlate", "insight", "recommend"] as const;

interface AUUseCaseSlideProps extends SlideNarrationProps {
  useCase: AutomationUseCase;
  slideNumber: number;
  id: string;
}

const AUUseCaseSlide = ({ useCase, slideNumber, id, ...props }: AUUseCaseSlideProps) => {
