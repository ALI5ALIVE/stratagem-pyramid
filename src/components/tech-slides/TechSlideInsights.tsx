...
import { Search, Lightbulb, TrendingUp, Zap, FileText, CheckCircle2, ChevronRight } from "lucide-react";
  ...
  >
    <div className="flex-1 flex flex-col gap-4 min-h-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        {capabilities.map((c) => (
          <div key={c.title} className={`rounded-xl border ${c.border} ${c.bg} p-4 flex flex-col`}>
      ...
      <div className="rounded-xl border border-primary/20 bg-card/40 p-4 flex-1 flex flex-col min-h-0">
        ...
        <div className="rounded-lg border border-primary/30 bg-primary/5 p-2.5 mb-3">
          <p className="text-xs text-foreground italic">"Are dangerous goods incidents linked to training gaps?"</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-2 flex-1">
          {[
            ...
            <div key={s.label} className="flex items-center gap-2">
              <div className={`flex-1 h-full rounded-lg border ${s.border} ${s.bg} p-2.5 flex flex-col`}>
