  ...
  <SalesSlideContainer id="tech-slide-8" title="Intelligence Tiers & Differentiation" subtitle="Four levels of operational intelligence — and why generic AI falls short" slideNumber={slideNumber} {...narrationProps}>
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-3 flex-1 min-h-0">
      ...
      <div className="lg:col-span-3 grid grid-cols-2 gap-2">
        {tiers.map((t) => (
          <div key={t.tier} className={`rounded-xl border ${t.border} ${t.bg} p-3 flex flex-col`}>
            <div className="flex items-center gap-2 mb-1.5">
              <t.icon className={`h-4 w-4 ${t.color}`} />
              <h3 className={`text-sm font-bold ${t.color}`}>{t.tier}</h3>
            </div>
            <p className="text-[10px] text-muted-foreground mb-2 flex-1">{t.desc}</p>
            <p className="text-[9px] text-muted-foreground/60 pt-1.5 border-t border-muted/10">{t.capability}</p>
          </div>
        ))}
      </div>
      ...
      <div className="lg:col-span-2 flex flex-col gap-2">
        <div className="rounded-xl border border-muted/20 bg-card/60 p-2.5 overflow-auto flex-1">
          <table className="w-full text-[10px]">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left py-1 text-[9px] text-muted-foreground font-medium">Capability</th>
                <th className="text-center py-1 text-[9px] text-primary font-bold w-16">CoAnalyst</th>
                <th className="text-center py-1 text-[9px] text-muted-foreground w-16">Generic</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((r, i) => (
                <tr key={i} className="border-b border-border/50">
                  <td className="py-0.5 text-[9px] text-muted-foreground">{r.cap}</td>
        ...
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-2.5">
          <h3 className="text-[9px] font-bold text-amber-400 uppercase tracking-wide mb-1.5">Accuracy by Depth</h3>
          {accuracy.map((a, i) => (
          ...
          <p className="text-[8px] text-muted-foreground mt-1.5 italic">LLMs hallucinate at granular levels — wrong technology for micro-categorization.</p>
        </div>
      </div>
    </div>
  </SalesSlideContainer>
