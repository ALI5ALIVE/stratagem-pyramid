...
const TechSlide9VsGenericAI = ({ slideNumber, ...narrationProps }: Props) => (
  <SalesSlideContainer id="tech-slide-9" title="CoAnalyst vs Generic AI" subtitle="Purpose-built aviation intelligence vs off-the-shelf AI tools" slideNumber={slideNumber} {...narrationProps}>
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-3 flex-1 min-h-0">
      <div className="lg:col-span-3 rounded-xl border border-muted/20 bg-card/60 p-3 overflow-auto">
        <table className="w-full text-[11px]">
          <thead><tr className="border-b border-border"><th className="text-left py-1.5 text-[10px] text-muted-foreground font-medium">Capability</th><th className="text-center py-1.5 text-[10px] text-primary font-bold w-20">CoAnalyst</th><th className="text-center py-1.5 text-[10px] text-muted-foreground w-20">Generic AI</th></tr></thead>
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} className="border-b border-border/50">
                <td className="py-1 text-[10px] text-muted-foreground">{r.cap}</td>
      ...
      <div className="lg:col-span-2 flex flex-col gap-2">
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-3">
          <h3 className="text-[10px] font-bold text-amber-400 uppercase tracking-wide mb-2">Accuracy by Depth</h3>
          {accuracy.map((a, i) => (
          ...
          <p className="text-[9px] text-muted-foreground mt-2 italic">LLMs hallucinate at granular levels — wrong technology for micro-categorization.</p>
        </div>
        <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-3">
          <h3 className="text-[10px] font-bold text-red-400 uppercase tracking-wide mb-2">Build vs Buy</h3>
          {buildVsBuy.map((b, i) => (<div key={i} className="text-[10px] text-muted-foreground mb-1">• {b}</div>))}
        </div>
        <div className="rounded-xl border border-primary/30 bg-primary/10 p-3 flex-1 flex flex-col justify-center">
          <p className="text-[11px] font-semibold text-foreground italic">"Generic AI reads reports. CoAnalyst understands aviation operations."</p>
        </div>
      </div>
    </div>
  </SalesSlideContainer>
