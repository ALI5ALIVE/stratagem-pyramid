    ...
    >
      <div className="flex-1 flex flex-col gap-2 h-full overflow-hidden">
        ...
        <div className="bg-primary/15 border-2 border-primary/40 rounded-xl px-5 py-2.5 text-center">
          <p className="text-xs text-primary uppercase tracking-widest font-semibold mb-0.5">Master Line</p>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-card/60 border border-border/50 rounded-lg px-3.5 py-2.5">
            <p className="text-xs text-primary uppercase tracking-wider font-semibold mb-0.5">Brand Promise</p>
            <p className="text-xs text-foreground leading-relaxed font-medium">
              Turn fragmented operational activity into connected performance.
            </p>
          </div>
          <div className="bg-card/60 border border-border/50 rounded-lg px-3.5 py-2.5">
            <p className="text-xs text-destructive uppercase tracking-wider font-semibold mb-0.5">The Problem We Solve</p>
        ...
        <div className="bg-accent/10 border border-accent/30 rounded-lg px-3.5 py-2.5">
          <p className="text-xs text-muted-foreground uppercase tracking-wider font-semibold mb-1">Operating Model — DTOP</p>
        ...
        {/* Messaging Pillars */}
        <div>
          <p className="text-xs text-primary uppercase tracking-wider font-semibold mb-1.5">Messaging Pillars</p>
              ...
              return (
                <div key={pillar.id} className="bg-card/60 border border-border/50 rounded-lg p-2.5 flex flex-col">
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <p className="text-xs font-bold text-foreground leading-tight">{pillar.title}</p>
                  </div>
                  <div className="space-y-0.5">
                    {pillar.points.map((pt) => (
                      <div key={pt} className="flex items-start gap-1">
                        <span className="w-1 h-1 rounded-full bg-primary/60 mt-1.5 shrink-0" />
                        <p className="text-xs text-foreground/70 leading-snug">{pt}</p>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div>
          <p className="text-xs text-primary uppercase tracking-wider font-semibold mb-1.5">Persona Pillars</p>
          <div className="grid grid-cols-5 gap-1.5">
            {personas.map((p) => {
              const Icon = p.icon;
              return (
                <div key={p.role} className="bg-card/60 border border-border/50 rounded-lg px-2.5 py-2 flex flex-col gap-0.5">
                  <div className="flex items-center gap-1">
                    <Icon className="w-3 h-3 text-primary shrink-0" />
                    <p className="text-xs font-bold text-primary leading-tight">{p.role}</p>
                  </div>
                  <p className="text-xs text-foreground/70 leading-snug">{p.message}</p>
                </div>
              );
            })}
          </div>
